// NOTE: If you run this file locally
// You will not get a server status and the example will fail
// Comment out lines 9 and 11 if you are working locally

var xhr = new XMLHttpRequest();                 // Create XMLHttpRequest object

xhr.onload = function() {                       // Run function when response has loaded
  // The following conditional check will not work locally - only on a server
  if(xhr.status === 200) {                       // Run if status returns ok
    document.getElementById('content').innerHTML = xhr.responseText; // Set content element equal to the returned text
  }
};

xhr.open('GET', 'data/data.html', true);        // Initializes a new request
xhr.send(null);                                 // Send request to the server
